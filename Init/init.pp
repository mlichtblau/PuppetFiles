class accounts {
  
  @accounts::virtual { 'admin':
    uid             =>  4000,
    realname        =>  'Ad Min',
    sshkeytype      =>  'ssh-rsa',
    sshkey          =>  ''
  }

  @accounts::virtual { 'user':
    uid             =>  4001,
    realname        =>  'Max Mustermann',
    sshkeytype      =>  'ssh-rsa',
    sshkey          =>  ''
  }

  @accounts::virtual { 'employee':
    uid             =>  4002,
    realname        =>  'Wissenschaftlicher Mitarbeiter',
    sshkeytype      =>  'ssh-rsa',
    sshkey          =>  ''
  }

  @accounts::virtual { 'ad.min':
    uid             =>  4003,
    realname        =>  'Ad Min',
    sshkeytype      =>  'ssh-rsa',
    sshkey          =>  ''
  }

  @accounts::virtual { 'hans.wurst':
    uid             =>  4004,
    realname        =>  'Hans Wurst',
    sshkeytype      =>  'ssh-rsa',
    sshkey          =>  ''
  }

  @accounts::virtual { 'son.horst':
    uid             =>  4005,
    realname        =>  'Son Horst',
    sshkeytype      =>  'ssh-rsa',
    sshkey          =>  ''
  }

  @accounts::virtual { 'soehnchen.horst':
    uid             =>  4006,
    realname        =>  'SohnDesSon Horst',
    sshkeytype      =>  'ssh-rsa',
    sshkey          =>  ''
  }

  @accounts::virtual { 'direx':
    uid             =>  4007,
    realname        =>  'Jochen Meinel',
    sshkeytype      =>  'ssh-rsa',
    sshkey          =>  ''
  }

}
